...
Oracle DB + Flyway 마이그레이션, Spring Security 인증 보호, 이메일 인증, 관리자 대시보드, 프로필 관리, **Quill.js 리치 텍스트 게시판** (이미지·동영상 업로드, 썸네일, 타입별 검색, 댓글, 좋아요), **알림 시스템**, **고정 공지 게시판**, **커스텀 역할 & 세부 기능 권한 시스템 (9종 Permission)**, **유저 사이드 패널**, **라이트/다크 테마**, **모바일 반응형 UI**, GitHub Actions CI/CD, Docker 컨테이너 배포까지 포함한 풀스택 구성입니다.
...
│   ├── HeaderAttributesAdvice.java        # @ControllerAdvice — 헤더 공통 model 속성 자동 주입
...
│   ├── Permission.java                    # 세부 기능 권한 enum (9종)
...
└── util/
    ├── AuthorityUtils.java                # UserDetails 기반 권한 체크 static 유틸 (isAdmin/hasAuthority/hasAnyAuthority)
    ├── PasswordValidator.java             # 비밀번호 정책 검증
    └── WebUtils.java                      # 공통 웹 유틸리티
...
| `POST_EDIT_OTHERS` | 타인 게시글 수정 | `BoardService.checkAuthor()` |
...
| `USER_LOCK_MANAGE` | 회원 계정 잠금 해제 | `/admin/users/*/unlock` |
| `USER_VERIFY_MANAGE` | 이메일 인증 강제 완료 | `/admin/users/*/verify` |
| `CUSTOM_ROLE_VIEW` | 커스텀 역할 목록 및 할당 회원 조회 | `GET /admin/roles/*/users` |

Spring Security `getAuthorities()`에서 커스텀 역할의 권한을 `PERM_{권한명}` 형태로 자동 변환.  
`@EnableMethodSecurity` 적용으로 `@PreAuthorize` 사용 가능.  
관리 관련 권한(`NOTICE_*`, `CATEGORY_MANAGE`, `USER_LOCK_MANAGE`, `USER_VERIFY_MANAGE`, `CUSTOM_ROLE_VIEW`) 중 하나라도 보유 시 헤더에 "관리 패널" 링크 표시.
...
- **수정**: 작성자·관리자·`POST_EDIT_OTHERS` 권한 보유자 가능
- **삭제**: 작성자·관리자·`POST_DELETE_OTHERS` 권한 보유자 가능
...
- 공간 부족 시 자동 숨김
- **모바일(640px 이하)**: 사이드 패널 숨김, 헤더 우측에 ☰ 햄버거 버튼 표시 → 드롭다운 메뉴로 전체 내비게이션 제공
...
- **회원 관리**
  - 시스템 권한 변경 (`ROLE_USER` ↔ `ROLE_ADMIN`) — ROLE_ADMIN 전용
  - 커스텀 역할 할당 (체크박스 모달, 복수 선택) — ROLE_ADMIN 전용
  - 회원 삭제, **비밀번호 초기화** (SecureRandom 12자리 → BCrypt 저장 → 이메일 발송) — ROLE_ADMIN 전용
  - 계정 잠금 해제 — ROLE_ADMIN 또는 `USER_LOCK_MANAGE` 권한 보유자
  - 이메일 인증 강제 완료 — ROLE_ADMIN 또는 `USER_VERIFY_MANAGE` 권한 보유자
  - 자기 자신 권한 변경 및 삭제 방지
- **커스텀 역할 관리**
  - 역할 추가/수정/삭제 (역할명, 설명, 세부 기능 권한 체크박스 선택) — ROLE_ADMIN 전용
  - **할당 회원 현황**: 역할별 할당 회원 수 표시, 클릭 시 모달로 할당 회원 목록 확인 및 역할 개별 해제
  - 커스텀 역할 목록 조회 — ROLE_ADMIN 또는 `CUSTOM_ROLE_VIEW` 권한 보유자
  - 역할 삭제 시 해당 역할이 할당된 회원은 자동 해제 (CASCADE)
- **공지 관리**: 등록/수정/삭제, 순서 변경(▲▼), 노출 여부 토글, 노출 게시판 다중 선택 — ROLE_ADMIN 또는 `NOTICE_WRITE`/`NOTICE_DELETE` 권한 보유자
- **카테고리 관리**: 게시판 카테고리 추가/수정/삭제, 정렬 순서 관리 — ROLE_ADMIN 또는 `CATEGORY_MANAGE` 권한 보유자
- **퀘이사존 수집**: 수동 트리거 + 자동 1시간 주기 크롤링 — ROLE_ADMIN 전용
- **권한별 섹션 표시**: 보유한 권한에 해당하는 관리 섹션만 노출 (관리 패널 링크도 동일)
- **모바일 반응형**: 768px 이하에서 회원 목록·공지 목록·커스텀 역할 목록을 카드 레이아웃으로 전환
...
### 모바일 반응형 UI
- 모든 페이지 `<meta name="viewport">` 적용
- **로그인 / 회원가입**: 카드 너비 `max-width: 360px` + 소형 화면 패딩 자동 축소
- **헤더 (640px 이하)**: 텍스트 버튼 숨김 → ☰ 햄버거 버튼 + 드롭다운 메뉴 (뒤로가기·내 작성글·프로필·로그아웃 등)
- **관리자 대시보드 (768px 이하)**: 회원 목록·공지 목록을 카드 레이아웃으로 전환 (아바타, 상태 배지, 액션 버튼 포함), 통계 카드 2열 표시
...
- `/admin/users/**`, `/admin/roles/**`, `/admin/import/**`는 `ROLE_ADMIN` 전용 (구체적 경로 예외 적용)
- `/admin/users/*/unlock`은 `ROLE_ADMIN` 또는 `PERM_USER_LOCK_MANAGE` 권한 보유자 접근 가능
- `/admin/users/*/verify`는 `ROLE_ADMIN` 또는 `PERM_USER_VERIFY_MANAGE` 권한 보유자 접근 가능
- `GET /admin/roles/*/users`는 `ROLE_ADMIN` 또는 `PERM_CUSTOM_ROLE_VIEW` 권한 보유자 접근 가능
...
| 날짜 | 내용 |
|------|------|
| 2026-04-10 | 코드 리팩토링 — `AuthorityUtils` 유틸 클래스 신설, 권한 체크 stream 보일러플레이트 일괄 제거 |
| 2026-04-10 | `AdminService.DashboardStats` record 추가 — 대시보드 통계 집계를 서비스 레이어로 이동 |
| 2026-04-10 | 세부 기능 권한 4개 추가 (`POST_EDIT_OTHERS`, `USER_LOCK_MANAGE`, `USER_VERIFY_MANAGE`, `CUSTOM_ROLE_VIEW`) |
| 2026-04-10 | 커스텀 역할 할당 회원 현황 조회 및 역할 개별 해제 기능 추가 (AJAX 모달) |
| 2026-04-10 | 관리 패널 — 보유 권한에 따라 관련 섹션만 노출 (헤더 "관리 패널" 링크 포함) |
| 2026-04-10 | SecurityConfig — 구체적 경로 우선 규칙 적용 (`unlock`, `verify`, `roles/*/users`) |
| 2026-04-10 | 관리자 대시보드 커스텀 역할 목록 모바일 카드 레이아웃 추가 |
